  ...
  try {
    ...
    return {
      products: Array.isArray(parsed.products)
        ? parsed.products
        : (Array.isArray(parsed.produk) ? parsed.produk : []),
      suhuMobil: parsed.suhuMobil ?? parsed.suhuMobilProduk ?? parsed.car ?? "",
      kebersihanMobil: parsed.kebersihanMobil ?? parsed.clean ?? "",
      kualitasProduk: parsed.kualitasProduk ?? parsed.quality ?? "",
      tindakanKoreksi: parsed.tindakanKoreksi ?? parsed.corr ?? "",
      keputusan: parsed.keputusan ?? parsed.decision ?? "",
      pic: parsed.pic ?? parsed.PIC ?? parsed.picName ?? parsed.namaPIC ?? ""
    };
  } catch (e) {
    return {
      products: [],
  ...
  sheetNames.forEach(function(sheetName) {
    ...
    groups.forEach(function(group) {
      ...
      result.push({
        id: manualId,
        category: category,
        // Fallback ke tanggal history jika kolom tanggal legacy kosong.
        tanggal: group.tanggal || filterDate,
 ...
 ****************************************************/
/****************************************************
 * LEGACY / MANUAL HISTORY COMPATIBILITY
 ****************************************************/
function parseLegacyId_(id) {
  const text = String(id || "").trim();
  const m = text.match(/^MANUAL-(HALAL|NONHALAL)-(\d{2})-(\d+)-/i);
  if (!m) return null;
  return {
    category: m[1].toUpperCase() === "HALAL" ? "HALAL" : "NON HALAL",
    sheet: m[2],
    startRow: Number(m[3]) || 0
  };
}

function findSheetGroupByStart_(sheet, startRow) {
  const groups = readPOGroupsFromSheet(sheet);
  return groups.find(function(group) {
    return Number(group.startRow) === Number(startRow);
  }) || null;
}

function ensureLegacyIndexRow_(ss, category, data) {
  const legacy = parseLegacyId_(data.id);
  if (!legacy) return null;

  const index = getIndexSheet(ss);
  const existing = findIndexRow(index, data.id);
  if (existing) return existing;

  const sheet = ss.getSheetByName(legacy.sheet);
  if (!sheet) throw new Error("Sheet tanggal " + legacy.sheet + " tidak ditemukan.");

  const group = findSheetGroupByStart_(sheet, legacy.startRow);
  if (!group) throw new Error("Transaksi lama tidak ditemukan di Spreadsheet.");

  const date = parseDate(data.tanggal);
  const newId = createTransactionId(date, category);
  ...
  saveIndex(ss, {
    ...
    endRow: group.endRow,
    separatorRow: group.separatorRow,
    noPo: group.noPo || data.noPo || "",
    vendor: group.vendor || data.vendor || "",
    products: group.products || [],
    suhuMobil: group.suhuMobil || data.suhuMobil || "",
    kebersihanMobil: group.kebersihanMobil || data.kebersihanMobil || "",
    kualitasProduk: group.kualitasProduk || data.kualitasProduk || "",
    tindakanKoreksi: group.tindakanKoreksi || data.tindakanKoreksi || "",
    keputusan: group.keputusan || data.keputusan || "",
    pic: data.pic || ""
  });

  return findIndexRow(index, newId);
}

function clearLegacyTransaction_(ss, category, data) {
  const legacy = parseLegacyId_(data.id);
  ...
  for (let r = group.startRow; r <= group.endRow; r++) clearRow(sheet, r);
  if (group.separatorRow >= group.startRow) clearRow(sheet, group.separatorRow);

  const index = getIndexSheet(ss);
  const last = index.getLastRow();
  if (last >= 2) {
    const rows = index.getRange(2, 1, last - 1, 13).getDisplayValues();
    rows.forEach(function(row, i) {
      const sheetName = String(row[3] || "").trim();
      const start = Number(row[4]) || 0;
      if (sheetName === legacy.sheet && start === legacy.startRow) {
        const rowNo = i + 2;
        index.getRange(rowNo, 13).setValue("DELETED");
        index.getRange(rowNo, 12).setValue(new Date());
      }
    });
  }
  return true;
...
function updateIncoming(data) {
  ...
  let indexRow = findIndexRow(index, data.id);

  if (!indexRow && String(data.id).toUpperCase().indexOf("MANUAL-") === 0) {
    indexRow = ensureLegacyIndexRow_(ss, category, data);
    if (!indexRow) throw new Error("Transaksi lama tidak ditemukan.");
    data.id = index.getRange(indexRow, 1).getDisplayValue();
  }
...
function deleteIncoming(data) {
  ...
  if (/^MANUAL-/i.test(idText)) {
    const legacyDate = data.tanggal || data.date;
    if (!legacyDate) throw new Error("Tanggal transaksi lama tidak ditemukan.");

    const ss = getSpreadsheet(category, parseDate(legacyDate));
    clearLegacyTransaction_(ss, category, data);
