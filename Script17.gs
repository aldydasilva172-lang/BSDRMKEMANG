 ...
 * GET SPREADSHEET
 ****************************************************/
function getSpreadsheet(category, tanggal) {
  ...
  return getMonthlySpreadsheet(category, date);
}
...
function getMonthlySpreadsheet(category, date) {
  category = normalizeCategory(category);
 ...
 * 31 → I31
 *
 * Pengelompokan berdasarkan NOMOR PO.
 * Semua produk dalam PO digabung dengan ", ".
 *
 * Target I:
 * B8:B9   = produk PO 1
 * B10     = supplier PO 1
 ...
 * B261:B264 / G261:G264
 ****************************************************/
function syncIncomingToI_(ss) {
  const targetBlocks = [
    8, 31, 54, 77, 100, 123,
    146, 169, 192, 215, 238, 261
  ];

  for (let day = 1; day <= 31; day++) {
    const sourceName = String(day).padStart(2, "0");
    const targetName = "I" + sourceName;
    ...
    // Baca seluruh PO dari Lembar 1-5.
    const groups = readPOGroupsFromSheet(sourceSheet);

    // Bersihkan hanya area target B/G yang memang dipakai.
    targetBlocks.forEach(function(startRow) {
      targetSheet.getRange(startRow, 2, 4, 1).clearContent(); // B
      targetSheet.getRange(startRow, 7, 4, 1).clearContent(); // G
    });

    // Setiap 2 PO masuk ke satu blok:
    // PO 1 -> B, PO 2 -> G, PO 3 -> B blok berikutnya, dst.
    groups.forEach(function(group, index) {
      if (index >= targetBlocks.length * 2) return;

      const blockIndex = Math.floor(index / 2);
      const startRow = targetBlocks[blockIndex];
      const column = (index % 2 === 0) ? 2 : 7;
      ...
      // Hilangkan duplikat tetapi pertahankan urutan.
      ...
      // Semua isi target memakai label + titik dua.
      // Produk: sel atas dari merge B8:B9 / G8:G9, dst.
      targetSheet.getRange(startRow, column)
        .setValue("Nama barang : " + productText);

      // Supplier: baris +2
      targetSheet.getRange(startRow + 2, column)
        .setValue("Nama supplier : " + (group.vendor || ""));

      // Tanggal: baris +3, format DD.MM.YYYY
      targetSheet.getRange(startRow + 3, column)
        .setValue("Tanggal : " + formatIIncomingDate_(group.tanggal));
    });
  }
...
/****************************************************
 * FORMAT TANGGAL UNTUK I01-I31
 ****************************************************/
function formatIIncomingDate_(value) {
  if (!value) return "";
 ...
 * SINKRONISASI MANUAL
 *
 * Bisa dijalankan sekali secara manual jika ingin
 * membangun ulang I01-I31 dari data yang sudah ada.
 *
 * Memproses file HALAL dan NON HALAL sesuai pilihan.
 ****************************************************/
function syncHalalIncomingToI() {
  const ss = SpreadsheetApp.openById(CONFIG.HALAL_ID);
  syncIncomingToI_(ss);
}

function syncNonHalalIncomingToI() {
  const ss = SpreadsheetApp.openById(CONFIG.NON_HALAL_ID);
  syncIncomingToI_(ss);
}
...
function saveIncoming(data) {
  ...
  // Update I01-I31 setelah data berhasil masuk ke sheet tanggal.
  syncIncomingToI_(ss);
...
function getHistory(category, tanggal) {
  ...
  const ss = getSpreadsheet(category, parseDate(filterDate));
...
function updateIncoming(data) {
  ...
  // Update I01-I31 setelah transaksi diperbarui.
  syncIncomingToI_(ss);
...
function deleteIncoming(data) {
  ...
  // Hapus/update data terkait di I01-I31 juga.
  syncIncomingToI_(ss);
