...
function updateIncoming(data) {
  ...
  const oldSs = getSpreadsheet(category, oldDate, false);
  if (!oldSs) throw new Error("Spreadsheet transaksi lama tidak ditemukan.");
  const oldIndex = getIndexSheet(oldSs);
  const indexRow = findIndexRow(oldIndex, data.id);
