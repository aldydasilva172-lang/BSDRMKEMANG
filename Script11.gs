...
function findAvailableBlock(sheet, requiredRows) {
  /*
   * PENTING:
   * Baris separator PO sebelumnya sengaja dikosongkan.
   * Jadi baris kosong tersebut TIDAK boleh dianggap sebagai
   * titik awal PO berikutnya.
   *
   * Kita ambil semua SEPARATOR_ROW yang masih ACTIVE dari
   * _INCOMING_INDEX, lalu blokir baris tersebut sebagai
   * reserved separator.
   */
  ...
  for (let b = 0; b < CONFIG.BLOCKS.length; b++) {
    ...
    for (let i = 0; i <= capacity - requiredRows; i++) {
      ...
      // Ini menjaga separator walaupun index belum/kurang lengkap.
      ...
      for (let j = 0; j < requiredRows; j++) {
        ...
        // Separator PO lama tidak boleh ditimpa.
