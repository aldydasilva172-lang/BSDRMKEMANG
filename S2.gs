...
function updateIncoming(data) {
  ...
  const located = findActiveTransactionRecord(category, data.id, oldDate);
...
function deleteIncoming(data) {
  ...
  const located = findActiveTransactionRecord(category, data.id, idDate);
  if (!located) throw new Error("Transaksi tidak ditemukan.");
 ...
 ****************************************************/
function findActiveTransactionRecord(category, id, fallbackDate) {
  category = normalizeCategory(category);
