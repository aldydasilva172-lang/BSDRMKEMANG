...
function getHistory(category, tanggal) {
  ...
  // Riwayat hanya membaca file yang sudah ada.
  // Jika file bulan tersebut sudah dihapus, jangan membuatnya kembali.
  ...
  /*
   * 1. Baca index web terlebih dahulu.
   *    Data yang sudah masuk melalui web tetap memakai ID,
   *    metadata, edit, dan delete seperti sebelumnya.
   */
  ...
  if (lastIndexRow >= 2) {
    const indexValues = index.getRange(
      2, 1, lastIndexRow - 1, 13
    ).getDisplayValues();
  ...
  /*
   * 2. Baca langsung sheet tanggal.
   *
   * Ini membuat perubahan manual di Spreadsheet ikut terlihat
   * ketika Riwayat Web di-refresh.
   *
   ...
   * K Keputusan
   *
   * BARIS PRODUK BERIKUTNYA:
   * A-C kosong, D-H berisi data produk.
   *
   * BARIS KOSONG:
   * separator antar PO.
   */
  const sheetNames = filterDate
    ? [Utilities.formatDate(parseDate(tanggal), Session.getScriptTimeZone(), "dd")]
    : CONFIG.BLOCKS.map(function(_, i) {
  ...
  sheetNames.forEach(function(sheetName) {
    ...
    groups.forEach(function(group) {
      ...
      const key = String(sheetName).trim() + "|" +
                  String(group.startRow).trim() + "|" +
                  String(group.noPo || "").trim();

      const indexed = indexMap[key];

      if (indexed) {
        /*
         * Untuk data web, tetap prioritaskan metadata index.
         * Products dari sheet diperbarui supaya perubahan manual
         * pada produk/qty/suhu juga ikut terlihat.
         */
        ...
        result.push({
          id: indexed.id,
          category: category,
          tanggal: indexed.tanggal || group.tanggal,
          sheet: sheetName,
          ...
          products: group.products.length ? group.products : (meta.products || []),
      ...
      /*
       * Data yang dibuat/edit langsung di Spreadsheet:
       * tampilkan juga di Riwayat Web.
       *
       * ID MANUAL dibuat deterministik dari sheet + row + PO,
       * sehingga refresh berikutnya tetap mengenali transaksi yang sama.
       */
      const manualId =
        "MANUAL-" +
...
function updateIncoming(data) {
  ...
  // Cari transaksi lama berdasarkan tanggal yang tertanam pada ID.
  // Ini membuat EDIT tetap bekerja walaupun tanggal/bulan diubah.
  const idMatch = String(data.id).trim().match(/^INC-(?:H|NH)-(\d{8})-/i);
  if (!idMatch) throw new Error("ID transaksi tidak valid.");
  const idDateText = idMatch[1];
  const oldDate = parseDate(
    idDateText.substring(0, 4) + "-" +
    idDateText.substring(4, 6) + "-" +
    idDateText.substring(6, 8)
  );

  const located = findActiveTransactionRecord(category, data.id, oldDate, data);
  if (!located) throw new Error("Transaksi tidak ditemukan.");

  const oldSs = located.ss;
  const oldIndex = located.index;
  const indexRow = located.row;

  const storedCategory = oldIndex.getRange(indexRow, 2).getDisplayValue();
  ...
  const oldSheetName = oldIndex.getRange(indexRow, 4).getDisplayValue();
  const oldStart = Number(oldIndex.getRange(indexRow, 5).getValue()) || 0;
  const oldEnd = Number(oldIndex.getRange(indexRow, 6).getValue()) || 0;
  const oldSeparator = Number(oldIndex.getRange(indexRow, 7).getValue()) || 0;

  let location = null;
  let responseIndexRow = indexRow;
  ...
  const createdValue = oldIndex.getRange(indexRow, 11).getValue();
  ...
  if (oldSs.getId() === ss.getId()) {
    // Bulan yang sama: cukup update record index yang sama.
    index.getRange(indexRow, 3, 1, 11).setValues(newIndexValues);
  } else {
    // Pindah bulan: nonaktifkan record lama, lalu buat record baru
    // dengan ID yang sama di file tujuan.
    oldIndex.getRange(indexRow, 13).setValue("DELETED");
    oldIndex.getRange(indexRow, 12).setValue(updatedNow);
    index.appendRow([
      data.id, category, formatDate(date), sheetName, location.start,
      location.start + products.length - 1, separator, data.noPo || "",
      data.vendor || "", JSON.stringify(payload), createdValue, updatedNow, "ACTIVE"
    ]);
    responseIndexRow = index.getLastRow();
  }
...
function deleteIncoming(data) {
  ...
  const located = findActiveTransactionRecord(category, data.id, idDate, data);
  if (!located) throw new Error("Transaksi tidak ditemukan.");
  ...
  return {success:true,message:"Data berhasil dihapus.",id:data.id,category:category};
}

/****************************************************
 * FIND ACTIVE TRANSACTION ACROSS MONTHLY FILES
 *
 * ID transaksi tetap memakai tanggal saat pertama dibuat.
 * Setelah EDIT pindah bulan, record lama menjadi DELETED
 * dan record ACTIVE dengan ID yang sama berada di file baru.
 * Fungsi ini mencari record ACTIVE tersebut, sehingga EDIT/
 * HAPUS tetap bekerja walaupun tanggal transaksi sudah berubah.
 ****************************************************/
function findActiveTransactionRecord(category, id, fallbackDate, hint) {
  category = normalizeCategory(category);
  const wanted = String(id || "").trim();
  if (!category || !wanted) return null;

  // FAST PATH: client mengirim lokasi index yang sudah diketahui.
  // Ini menghindari scan semua file bulanan untuk EDIT/DELETE.
  hint = hint || {};
  if (hint.spreadsheetId && Number(hint.indexRow) >= 2) {
    try {
      const fastSs = SpreadsheetApp.openById(String(hint.spreadsheetId));
      const fastIndex = getIndexSheet(fastSs);
      const fastRow = Number(hint.indexRow);
      const rowValues = fastIndex.getRange(fastRow, 1, 1, 13).getDisplayValues()[0];
      if (String(rowValues[0]).trim() === wanted &&
          String(rowValues[1]).trim() === category &&
          String(rowValues[12]).trim().toUpperCase() === "ACTIVE") {
        return {ss: fastSs, index: fastIndex, row: fastRow};
      }
    } catch (e) {
      // Lanjut ke fallback lama bila hint sudah tidak valid.
    }
  }
  ...
  // File bulan asal dari tanggal yang tertanam pada ID.
  try { addSpreadsheet(getSpreadsheet(category, fallbackDate, false)); } catch(e) {}

  // Semua file bulanan yang sudah diregistrasikan oleh sistem.
  const props = PropertiesService.getScriptProperties().getProperties();
  const prefix = "INCOMING_MONTHLY_" + category.replace(/\s+/g, "_") + "_";
  Object.keys(props).forEach(function(key) {
    if (!key.startsWith(prefix)) return;
    const idFile = props[key];
    if (!idFile || seen[idFile]) return;
    try { addSpreadsheet(SpreadsheetApp.openById(idFile)); } catch(e) {}
  });

  // Fallback untuk file lama yang belum tercatat di Properties.
  const suffix = category === "HALAL" ? "HALAL" : "NON HALAL";
  const prefixName = "FORM INCOMING BSD " + suffix + " ";
  try {
    const files = DriveApp.getFiles();
    while (files.hasNext()) {
      const file = files.next();
      if (file.getName().startsWith(prefixName)) addSpreadsheet(SpreadsheetApp.openById(file.getId()));
    }
  } catch(e) {}

  // Prioritaskan record ACTIVE. Jika ada duplicate ID aktif karena
  // histori edit lintas bulan, gunakan record ACTIVE terbaru.
  let found = null;
  let latest = 0;

  candidates.forEach(function(ss) {
    try {
      const index = getIndexSheet(ss);
      const last = index.getLastRow();
      if (last < 2) return;
      const values = index.getRange(2,1,last-1,13).getDisplayValues();
      values.forEach(function(row,i) {
        if (String(row[0]).trim() !== wanted) return;
        if (String(row[12]).trim().toUpperCase() !== "ACTIVE") return;
        const stamp = new Date(row[11] || row[10] || 0).getTime() || 0;
        if (!found || stamp >= latest) {
          found = {ss:ss,index:index,row:i+2};
          latest = stamp;
        }
      });
    } catch(e) {}
...
function validateIncoming(data) {
  ...
  if (!String(data.pic || "").trim()) throw new Error("PIC wajib dipilih.");
