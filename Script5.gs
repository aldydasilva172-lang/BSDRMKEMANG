...
function writeRow(sheet, row, data) {
  ...
  const firstProduct = data.firstProduct !== false;

  // Data level PO hanya pada baris pertama.
  // Baris produk berikutnya hanya berisi data produknya.
  const values = [[
    firstProduct ? (data.tanggal || "") : "",
    firstProduct ? (data.noPo || "") : "",
    firstProduct ? (data.vendor || "") : "",
    p.name || "",
    p.qty ?? "",
    firstProduct ? (data.suhuMobil || "") : "",
    firstProduct ? (data.kebersihanMobil || "") : "",
    p.suhuProduk || "",
    firstProduct ? (p.kualitasProduk || "") : "",
    firstProduct ? (p.tindakanKoreksi || "") : "",
    firstProduct ? (p.keputusan || "") : "",
    "",
    ""
  ]];
